...
void CameraUpdater::update_1st_person(float forward, float right, float dx, float dy) {
    ...
    const static float sens = 0.0002;
    ...
    // and is independent of the screen size
    ...
    float cos_pitch = cos(prs->pitch); float cos_yaw = cos(prs->yaw);
    float sin_pitch = sin(prs->pitch); float sin_yaw = sin(prs->yaw);
