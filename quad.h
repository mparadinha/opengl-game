...
public:
    ...
    Quad(float size, unsigned int density) :
        Mesh(get_vertices(size, density), get_indices(density), std::vector<texture_t>()) {};
...
private:
    ...
    std::vector<vertex_t> get_vertices(float size, unsigned density) {
